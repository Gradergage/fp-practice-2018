...
-- тогда для этого композиция функций тоже изменится и станет a . f. Эта самая смена направления стрелочек,
-- а по-человечески - смена отношения порядка на противоположное - называется контравариантность,
-- если отношение порядка сохраняется - ковариантность.
-- Грубо говоря получится вот такая вещь ((a -> b) -> bool), что полностью нас устраивает
-- Таким образом, ковариантного функтора, для PSet по определению чистой fmap (a -> b) -> PSet a -> PSet b не существует,
...
-- Если изменить изначальный тип на отображение СontraPSet (Bool -> a),
-- то становится доступным ковариантный функтор с fmap (a -> b) -> СontraPSet a -> СontraPSet b, в котором стрелочки,
